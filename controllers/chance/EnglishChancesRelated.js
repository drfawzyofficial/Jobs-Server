...
const EnglishChancesRelated = async (req, res) => {
    try {
        const user = await Student.findById(req.user._id);
        const chance = await Chance.findById(req.body._id);
        if(!chance)
            return sendResponse(res, 404, "الفرصة غير موجودة");
        let search_query = {applicantEdus: { $in: [user.applicantEdu] }, chanceSubcategory: "إنجليزي", _id: { $ne: req.body._id } };
...
// Export chanceGet
module.exports = EnglishChancesRelated;